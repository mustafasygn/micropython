...
int mp_hal_stdin_rx_chr(void) {
    for (;;) {
#if 0
#ifdef USE_HOST_MODE
        leaf_usb_host_process();
        int c = leaf_usb_host_get_keyboard();
        if (c != 0) {
            return c;
        }
#endif
        ...
        if (usb_vcp_recv_byte(&c) != 0) {
            return c;
        } else if (MP_STATE_PORT(leaf_stdio_uart) != NULL && uart_rx_any(MP_STATE_PORT(leaf_stdio_uart))) {
            return uart_rx_char(MP_STATE_PORT(leaf_stdio_uart));
        }
...
void mp_hal_stdout_tx_strn(const char *str, size_t len) {
    if (MP_STATE_PORT(leaf_stdio_uart) != NULL) {
        uart_tx_strn(MP_STATE_PORT(leaf_stdio_uart), str, len);
    }
